...
public class ConfigLocation : ConfigWidget
{
  ...
  void insert_google_drive() {
    insert_cloud_if_available("google-drive", BackendGoogleDrive.get_checker(),
                              new ThemedIcon("deja-dup-cloud"),
                              _("Google Drive"),
                              new ConfigLocationGoogleDrive(label_sizes),
                              ref index_google_drive, insert_google_drive);
  }
  ...
  protected override async void set_from_config()
  {
    ...
    if (backend == "s3")
      index = index_s3;
    else if (backend == "google-drive")
      index = index_google_drive;
  ...
  async void set_location_info()
  {
    ...
    else if (index == index_google_drive)
      settings.set_string(BACKEND_KEY, "google-drive");
